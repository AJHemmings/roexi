...
export function Spinner({ className = 'w-3 h-3' }: { className?: string }) {
  return <span role="status" aria-label="Working" className={`inline-block shrink-0 rounded-full border-2 border-current border-r-transparent motion-safe:animate-spin ${className}`} />;
}
